...
Добавьте переменную окружения YANDEX_GEO_API_KEY с вашим ключом [инструкция получения](https://dvmn.org/encyclopedia/api-docs/yandex-geocoder-api/).
```sh
YANDEX_GEO_API_KEY=Ваш ключ
```
...
- `ROLLBAR_ENABLED` - Подключать Rollbar или нет

    Если ROLLBAR_ENABLED=True, то необходимо указать следующие переменные в `.env`
